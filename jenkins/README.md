...
# Ensure ec2:RunInstances is allowed

# Check Jenkins system log
# Manage Jenkins → System Log

# Verify AMI exists and is available in the region
aws ec2 describe-images --image-ids ami-xxxxx
```

### Issue: Connection Timeout
```bash
# Verify security group allows SSH from master
# Check subnet has internet gateway route
# Verify key pair is correct

# Test manual SSH from Jenkins master:
ssh -i /var/lib/jenkins/secrets/jenkins-agent.pem ubuntu@<AGENT_IP>
```

### Issue: Agent Disconnects Immediately
```bash
# Check Java installation on agent
# Verify init script completes successfully
# Check /var/log/cloud-init-output.log on agent
```
...
- [ ] AMI template configured
- [ ] Security groups properly set
- [ ] Test job runs successfully on EC2 agent
- [ ] Agents terminate after idle timeout
...
│  │ t3.small    │  │ t3.small    │  │ t3.large    │     │
│  │ Running     │  │ Running     │  │ Running     │     │
│  │ Build #1    │  │ Build #2    │  │ Build #3    │     │
│  └─────────────┘  └─────────────┘  └─────────────┘     │
│                                                         │
│  ← Auto-terminated after 30 min idle                   │
└─────────────────────────────────────────────────────────┘
```
...
| Set idle timeout to 10 min | Reduce running time |
| Use smaller instance types | Base cost reduction |
| Schedule agents (stop at night) | 50% or more |
| Pre-configured AMIs | Faster startup |

---

## Next Steps
- Set up EKS plugin for Kubernetes agents
- Configure pipeline shared libraries
...
- [Jenkins on AWS](https://www.jenkins.io/doc/book/installing/kubernetes/)
