...
### Issue: Can't SSH to Public Instance
```bash
# Check:
1. Security group allows SSH (port 22) from your IP
2. Instance is in public subnet with IGW route
3. Key pair permissions (chmod 400)
4. Correct username (ec2-user for Amazon Linux, ubuntu for Ubuntu)
```
...
2. Private subnet route table points to NAT GW
3. NAT Gateway is in "available" state
```

### Issue: Can't SSH from Public to Private
```bash
# Check:
1. Private security group allows SSH from Public SG
2. Using private IP (not public - private has none)
3. Same key pair for both instances
```

### Issue: VPC DNS Resolution
```bash
# Verify VPC settings:
# VPC → Actions → Edit VPC Settings
...
- [ ] Public EC2 launched with public IP
- [ ] Private EC2 launched without public IP
- [ ] Can SSH to public instance
- [ ] Can SSH from public to private
- [ ] Private instance has internet via NAT
...
| Traffic Type | Path | Components |
|--------------|------|------------|
| **Inbound HTTP** | Internet → IGW → Public Subnet → EC2 | Security Group allows 80/443 |
| **Outbound from Public** | EC2 → Public RT → IGW → Internet | 0.0.0.0/0 → IGW |
| **Outbound from Private** | EC2 → Private RT → NAT GW → IGW → Internet | 0.0.0.0/0 → NAT |
| **SSH to Private** | User → Public EC2 → Private EC2 | Security group chaining |
...
4. Delete VPC (removes IGW, subnets, route tables, SGs)
```
